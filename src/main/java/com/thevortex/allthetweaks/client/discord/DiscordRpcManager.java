package com.thevortex.allthetweaks.client.discord;
...
public final class DiscordRpcManager {
    ...
    public static void start() {
        if (ModList.get().isLoaded("simplerpc")) {
            ...
            AllTheTweaks.LOGGER.info("Simple RPC is installed, leaving Discord Rich Presence to it");
            return;
        }

        synchronized (DiscordRpcManager.LOCK) {
            if (DiscordRpcManager.started) {
                AllTheTweaks.LOGGER.debug("Cannot start, already started");
                return;
            }
        ...
        AllTheTweaks.LOGGER.info("AllTheTweaks Discord RPC Manager started");
    }
    ...
    public static void onClientTick(ClientTickEvent.Post event) {
        ...
        try {
            ...
            if (!DiscordRpcManager.refreshFailureLogged) {
                ...
                AllTheTweaks.LOGGER.warn("Discord RPC refresh failed, further failures are logged at debug", exception);
            } else {
                AllTheTweaks.LOGGER.debug("Discord RPC refresh failed", exception);
            }
        }
    }
    ...
    private static void refreshFromConfigOnRenderThread() {
    ...
    private static void scheduleUpdate() {
        ...
        try {
            ...
            AllTheTweaks.LOGGER.debug("Discord RPC worker rejected update", exception);
        }
    }
    ...
    private static Snapshot createSnapshot() {
        ...
        String mods = ModList.get().size() + " Mods";
        ...
        synchronized (DiscordRpcManager.CLIENT_LOCK) {
            ...
            try {
                ...
                if (!DiscordRpcManager.connectLogged) {
                    ...
                    AllTheTweaks.LOGGER.info("Discord Rich Presence connected as {} (application {})",
                ...
                if (!DiscordRpcManager.connectFailureLogged) {
                    ...
                    AllTheTweaks.LOGGER.warn("Discord Rich Presence unavailable, retrying every {} seconds, further failures are logged at debug: {}",
                            DiscordRpcManager.RECONNECT_DELAY_MS / 1000L, exception.getMessage());
                } else {
                    AllTheTweaks.LOGGER.debug("Discord RPC unavailable: {}", exception.getMessage());
                }

                AllTheTweaks.LOGGER.trace("Discord RPC connection failure stack trace", exception);
                return false;
            }
        }
    }
    ...
    private static Activity buildActivity(Snapshot snapshot) {
        ...
        if (!snapshot.curseforgeUrl().isBlank()) {
            builder.addButton("CurseForge", snapshot.curseforgeUrl());
        }
    ...
    private static void handleFailure(Exception exception) {
        ...
        AllTheTweaks.LOGGER.debug("Discord RPC update failed: {}", exception.getMessage());
        AllTheTweaks.LOGGER.trace("Discord RPC update failure stack trace", exception);
    }
    ...
    private static void closeClient() {
        ...
        try {
            ...
            AllTheTweaks.LOGGER.debug("Failed to clear Discord RPC activity", exception);
        }

        try {
            currentClient.close();
        } catch (Exception exception) {
            AllTheTweaks.LOGGER.debug("Failed to close Discord RPC client", exception);
        }
    }
    ...
    private static boolean isConfiguredEnabled() {
        ...
        return Configuration.COMMON.discord.get() && Configuration.CLIENT.rpcEnabled.get();
    }
